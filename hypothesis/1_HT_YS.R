...
#Plot the distribution of FVC for each group
ggplot(synthetic_data, aes(x = Group, y = FVC, fill = Group)) +
  geom_boxplot() +
  labs(title = "Distribution of FVC for Smokers and Non-Smokers",
       y = "FVC (Litres)",
       x = "Group") +
  theme_minimal()
...
df <- data.frame(
  FVC = c(FVC_smokers, FVC_non_smokers),
  Group = rep(c("Smokers", "Non-Smokers"), times = c(n_smokers, n_non_smokers))
)

ggplot(df, aes(x = FVC, fill = Group)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = mean_smokers, color = "orange", linetype = "dashed", size = 1.2) +
  geom_vline(xintercept = mean_non_smokers, color = "blue", linetype = "dashed", size = 1.2) +
  labs(title = "Distribution of FVC for Smokers and Non-Smokers",
       x = "Forced Vital Capacity (FVC)",
       y = "Density",
       fill = "Group") +
  theme_minimal()
...
cat("Type II Error (Beta):", round(beta, 3), "\n")
